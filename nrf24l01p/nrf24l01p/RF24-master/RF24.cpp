...
#define AVR_SPI_MSTR_INIT() spi_master_initialize();
#define AVR_SPI_MSTR_TRANSMIT_VAL spi_master_transmit_byte_val
...
#elif defined(__AVR__)
	  beginTransaction();
	  status = AVR_SPI_MSTR_TRANSMIT_VAL( R_REGISTER | ( REGISTER_MASK & reg ) );
	  while ( len-- ){
		  *buf++ = AVR_SPI_MSTR_TRANSMIT_VAL(0xff);
	  }
	  endTransaction();
...
uint8_t RF24::read_payload(void* buf, uint8_t data_len)
{
  ...
  IF_SERIAL_DEBUG( printf("[Reading %u bytes %u blanks]\n",data_len,blank_len); );
	...
	*current = *prx;
	...
	while ( blank_len-- ) {
		AVR_SPI_MSTR_TRANSMIT_VAL(0xff);
	}
	endTransaction();
...
uint8_t RF24::getDynamicPayloadSize(void)
{
  ...
  #if defined(__AVR__)
  AVR_SPI_MSTR_TRANSMIT_VAL( R_RX_PL_WID );
  result = AVR_SPI_MSTR_TRANSMIT_VAL(0xff);
  #else
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  #elif defined (__AVR__)
    beginTransaction();
    AVR_SPI_MSTR_TRANSMIT_VAL(W_ACK_PAYLOAD | ( pipe & 0b111 ) );
